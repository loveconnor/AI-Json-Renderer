...
import { ReactFlowWidget, ReactFlowWidgetData } from "./react-flow-widget";
import { ChartWidget, ChartWidgetData } from "./chart-widget";
...
const DEFAULT_COMPONENTS: Partial<Components> = {
  h1: ({ children }) => (
    <h1 className="text-lg font-bold mt-4 mb-3">{children}</h1>
  ),
  h2: ({ children }) => (
  ...
  p: ({ children, node }) => {
    ...
    const hasBlockChild = React.Children.toArray(children).some((child) => {
      ...
      if (typeof childType === "function") {
        const displayName =
          (childType as any).displayName || childType.name || "";
        return (
          displayName.includes("CodeBlock") || displayName.includes("Table")
        );
      }
      // Check for HTML block elements
      return ["div", "pre", "table", "blockquote", "ul", "ol", "hr"].includes(
        String(childType),
      );
    });
    ...
    if (isInline) {
      ...
      if (isMath) {
        ...
        return (
          <code
            className={cn(
              "bg-muted/60 text-foreground rounded px-1.5 py-0.5 font-mono border border-border/50",
              className,
            )}
          >
            {mathContent}
...
function MarkdownComponent({
  ...
  const { processedContent, visualComponents, chartComponents } =
    useMemo(() => {
      ...
      while ((match = visualTagPattern.exec(processedText)) !== null) {
        try {
          ...
          processedText =
            processedText.substring(0, startIdx) +
            marker +
            processedText.substring(endIdx);
    ...
    () => ({
      ...
      p: ({ children, node }: any) => {
          ...
          .map((child) => {
            ...
            if (React.isValidElement<{ children?: React.ReactNode }>(child)) {
              ...
              if (childChildren != null) {
                return React.Children.toArray(childChildren)
                  .map((item) =>
        ...
        if (textContent) {
          ...
          if (visualMatch) {
            const visualIndex = Number(visualMatch[1] ?? "0");
          ...
          if (chartMatch) {
            const chartIndex = Number(chartMatch[1] ?? "0");
  ...
  return (
    <div
      className={cn(
        "prose prose-neutral dark:prose-invert max-w-none prose-headings:font-semibold prose-p:leading-relaxed prose-pre:bg-muted prose-pre:border prose-pre:overflow-x-auto break-words prose-code:before:content-none prose-code:after:content-none",
        className,
      )}
    >
      <ReactMarkdown
...
function MarkdownRenderer({ element }: ComponentRenderProps) {
  const props = element.props as { content?: string; className?: string };
  return (
    <Markdown className={props.className ?? undefined}>
      {props.content ?? ""}
    </Markdown>
  );
}

export { Markdown, MarkdownRenderer };
